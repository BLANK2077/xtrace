...
- **Agent 友好入口** — `session ensure -dbdir ... --name <name> [-json]` 创建具名 Session，`query --name <name>` 可一次完成启动 + trace
- **僵死 Session 自动清理** — 自动检测并回收异常退出的 Session
- **Session 健康诊断** — `session doctor -s <name> [-json]` 区分 registry 缺失、进程退出、socket 缺失、连接失败和 PING 失败
- **Daidir 指纹校验** — `open` 只接受 `-dbdir <*.daidir>`，并记录 canonical path、mtime、size、dev、inode
- **定向命令** — 支持通过 `-s <name>` 向指定 Session 发送命令
...
Session 创建或健康检查失败时，可使用 `--debug` 或 `XTRACE_DEBUG=1`。debug 诊断会写入 stderr，server 侧启动细节写入 `~/.xtrace/sessions/<hashed-name>/debug.log`。
...
# 用源码 grep 查候选，再把完整路径交给 xtrace 解析
rg -n "ready" /path/to/rtl
```

`signal.resolve` 只接受精确 RTL 路径；找不到信号时返回非 0，并给出 `status=not_found`。短名或片段查找交给外部 `rg`/grep，不在 xtrace 内部读取 daidir 文本实现 search。
...
- 不健康 Session 返回非 `0`
- 缺少 `-s <name>` 或非法 Session 名称 返回非 `0`
- JSON 输出包含字符串 `id`/`session_id`、`healthy`、`status`、`message`、`pid`、`socket_path`、`design_file`、`dbdir_path` 和 daidir metadata
